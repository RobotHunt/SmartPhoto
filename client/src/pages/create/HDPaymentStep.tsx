        ...
        <div className="glass-panel border-slate-200 rounded-[32px] overflow-hidden shadow-[0_0_50px_rgba(34,211,238,0.15)] flex flex-col">
          ...
          <div className="flex items-center gap-3 px-6 py-5 bg-slate-100 border-b border-slate-200">
            <button
              ...
              className="w-8 h-8 flex items-center justify-center rounded-full bg-white/80 border border-slate-200 text-slate-600 hover:text-slate-900 hover:bg-slate-200 transition"
            >
              <ArrowLeft className="w-4 h-4" />
            </button>
            <h1 className="text-lg font-bold tracking-widest text-slate-900">生成无水印高清图</h1>
          </div>
          ...
          <div className="p-6 sm:p-8">
            ...
            <div className="flex items-center gap-2 mb-6 bg-white/80 border border-cyan-200 px-4 py-2 rounded-xl inline-flex shadow-sm">
              ...
              <span className="text-sm font-bold tracking-widest text-cyan-800">
                共 <span className="text-cyan-600 px-1">{selectedCount}</span> 张图片
              </span>
            </div>
            ...
            <div className="flex gap-3 mb-8">
              {loading ? (
                <>
                  {Array.from({ length: Math.min(PREVIEW_MAX, selectedCount || PREVIEW_MAX) }).map((_, i) => (
                    <div key={i} className="shrink-0 w-24 h-24 rounded-2xl border border-slate-200 bg-white/80 animate-pulse shadow-[inset_0_2px_10px_rgba(0,0,0,0.1)]" />
                  ))}
                </>
              ) : (
                <>
                  {previewImages.map((src, i) => (
                    <div key={i} className="relative shrink-0 w-24 h-24 rounded-2xl overflow-hidden border border-slate-200 shadow-[0_4px_12px_rgba(0,0,0,0.1)]">
              ...
              {remaining > 0 && (
                <div className="shrink-0 w-24 h-24 rounded-2xl border border-slate-200 bg-white/80 flex flex-col items-center justify-center shadow-[inset_0_2px_10px_rgba(0,0,0,0.1)]">
                  <span className="text-xl font-black text-slate-600">+{remaining}</span>
